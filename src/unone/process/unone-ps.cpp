...
/*++
Description:
	read process console output
Arguments:
	cmdline
	output
	exitcode
	flag - FLAG_STDERR | FLAG_STDOUT
	timeout
Return:
	bool
--*/
bool PsReadConsoleDataA(const std::string& cmdline, std::string& output, DWORD* exitcode/*NULL*/, DWORD flag, DWORD timeout /*= INFINITE*/)
{
	return PsReadConsoleDataW(UNONE::StrToW(cmdline), output, exitcode, timeout);
}
bool PsReadConsoleDataW(const std::wstring& cmdline, std::string& output, DWORD* exitcode/*NULL*/, DWORD flag, DWORD timeout /*= INFINITE*/)
{
	...
	if (flag | FLAG_STDOUT) si.hStdOutput = wstdout;
	if (flag | FLAG_STDERR) si.hStdError = wstdout;
